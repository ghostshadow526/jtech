...
export default function LoginCardSection({
  ...
  return (
    <section className="fixed inset-0 bg-zinc-950 text-zinc-50 z-[9999]">
      <style>{`
      ...
      <header className="absolute left-0 right-0 top-0 flex items-center justify-between px-6 py-4 border-b border-zinc-800/80 z-10">
        ...
        <Button
          ...
          onClick={() => window.location.href = 'mailto:support@jttech.com'}
        >
        ...
        <Card className="card-animate w-full max-w-sm border-zinc-800 bg-zinc-900/70 backdrop-blur supports-[backdrop-filter]:bg-zinc-900/60">
          <CardHeader className="space-y-1">
            ...
            <CardDescription className="text-zinc-400">
              {mode === 'login' ? 'Sign in to your account' : 'Join JT Technologies today'}
            </CardDescription>
          </CardHeader>
          ...
          <CardContent className="grid gap-5">
            <form onSubmit={handleSubmit} className="grid gap-5">
              ...
              >
                {isLoading ? 'Processing...' : (mode === 'login' ? 'Continue' : 'Sign Up')}
              </Button>
            </form>
